                    ...
                    <x-dropdown align="right" width="48">
                        <x-slot name="trigger">
                            <button class="flex items-center text-sm border-2 border-transparent rounded-full focus:outline-none focus:border-gray-300 transition duration-150 ease-in-out">
                                <div class="font-medium text-gray-500">{{ Auth::user()->name }}</div>
    ...
    <div class="md:hidden" x-show="open" @click.away="open = false">
        <div class="pt-2 pb-3 space-y-1">
            ...
            @if(auth()->user()->isStudent())
                <x-responsive-nav-link :href="route('tasks.student.index')" :active="request()->routeIs('tasks.student.*')">
                    My Tasks
                </x-responsive-nav-link>
                <x-responsive-nav-link :href="route('announcements.student.index')" :active="request()->routeIs('announcements.student.*')">
                    Announcements
                </x-responsive-nav-link>
            @endif
